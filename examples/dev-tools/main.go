// Package main demonstrates integration with development tools and IDEs.
// This example shows how to create developer utilities that need system access.
package main
...
func main() {
	...
	// Configure macgo for development tools
	macgo.SetAppName("DevTools")
	macgo.SetBundleID("com.example.devtools")

	// Request permissions for development activities
	macgo.RequestEntitlements(
		macgo.EntAppSandbox,
		macgo.EntUserSelectedReadWrite, // Read/write project files
		macgo.EntNetworkClient,         // Package downloads, API calls
		macgo.EntNetworkServer,         // Development servers
		macgo.EntShell,                 // Execute build tools
	)
...
func buildProject(project *ProjectInfo) *BuildResult {
	...
	case "Python":
		...
		result.Success = true
		result.Output = "Python project (no build required)"
		return result
	default:
...
func watchProject(project *ProjectInfo, autoFormat, autoLint bool) {
	fmt.Printf("👁 Watching project for changes...\n")
