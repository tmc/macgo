// Package main demonstrates media processing with hardware access permissions.
// This example shows how to process audio/video files with hardware acceleration.
package main
...
func main() {
	...
	// Configure macgo for media processing
	macgo.SetAppName("MediaProcessor")
	macgo.SetBundleID("com.example.mediaprocessor")

	// Request permissions for media processing
	entitlements := []interface{}{
		macgo.EntAppSandbox,
		macgo.EntUserSelectedReadWrite, // Read/write media files
		macgo.EntHardwareVideoDecoder,  // Hardware video decoding
		macgo.EntHardwareVideoEncoder,  // Hardware video encoding
	}

	// Add permissions for live capture if needed
	...
	// Hide from dock for CLI usage
	macgo.AddPlistEntry("LSUIElement", true)

	// Add usage descriptions
	macgo.AddPlistEntry("NSCameraUsageDescription", "This app needs camera access to capture video.")
	macgo.AddPlistEntry("NSMicrophoneUsageDescription", "This app needs microphone access to capture audio.")
...
func processMedia(input, output string, opts *ProcessOptions) {
	...
	if opts.HardwareAccel {
		fmt.Println("   Hardware acceleration: enabled")
	}
...
func processBatch(inputDir, outputDir string, opts *ProcessOptions) {
	...
	fmt.Printf("✅ Batch processing completed: %d file(s)\n", len(files))
}
