// Package main demonstrates a background service/agent that runs without a dock icon.
// This example shows how to create a long-running background process with system tray integration.
package main
...
func main() {
	...
	// Configure macgo for background operation
	macgo.SetAppName("BackgroundAgent")
	macgo.SetBundleID("com.example.backgroundagent")

	// Request permissions for background agent
	macgo.RequestEntitlements(
		macgo.EntAppSandbox,
		macgo.EntUserSelectedReadOnly, // Read config and watch files
		macgo.EntNetworkClient,        // Send notifications or metrics
	)

	// Configure as background service (no dock icon, no menu bar)
	macgo.AddPlistEntry("LSUIElement", true)        // Hide from dock
	macgo.AddPlistEntry("LSBackgroundOnly", true)   // Background only
	macgo.AddPlistEntry("NSSupportsSuddenTermination", false) // Graceful shutdown

	// Add launch agent keys for auto-start (if installed as launch agent)
	macgo.AddPlistEntry("RunAtLoad", true)
	macgo.AddPlistEntry("KeepAlive", true)
	...
	fmt.Printf("🤖 Background Agent Started\n")
	fmt.Printf("   PID: %d\n", os.Getpid())
	fmt.Printf("   Logs: %s\n", logFile)
	...
	if *daemon {
		// Note: Full daemonization would require additional steps
