package contacts

import (
	"testing"

	"github.com/tmc/misc/macgo"
	"github.com/tmc/misc/macgo/entitlements"
)

func TestContactsPackageInit(t *testing.T) {
	// Test that the contacts entitlement is registered during package init
	// Since init() already ran when the package was imported, we simulate this
	// by creating a fresh config and calling the registration manually

	// Create a backup of the original config
	originalConfig := macgo.DefaultConfig
	defer func() {
		macgo.DefaultConfig = originalConfig
	}()

	// Reset config to clean state
	macgo.DefaultConfig = macgo.NewConfig()

	// Simulate what init() does
	macgo.RequestEntitlement(entitlements.EntAddressBook)

	// Check if it's registered
	expectedEntitlement := macgo.Entitlement(entitlements.EntAddressBook)
	if val, ok := macgo.DefaultConfig.Entitlements[expectedEntitlement]; !ok || !val {
		t.Errorf("Expected contacts entitlement %s to be registered as true during package init", expectedEntitlement)
	}
}

func TestContactsEntitlementValue(t *testing.T) {
	// Verify that the contacts entitlement constant has the correct value
	expected := "com.apple.security.personal-information.addressbook"
	if string(entitlements.EntAddressBook) != expected {
		t.Errorf("Expected contacts entitlement to be %q, got %q", expected, string(entitlements.EntAddressBook))
	}
}

func TestContactsEntitlementMatchesMainPackage(t *testing.T) {
	// Verify that our contacts entitlement matches the main package
	if string(entitlements.EntAddressBook) != string(macgo.EntAddressBook) {
		t.Errorf("Contacts entitlement mismatch: entitlements package %q != macgo package %q",
			entitlements.EntAddressBook, macgo.EntAddressBook)
	}
}

func TestPackageImportSideEffect(t *testing.T) {
	// Check that the contacts entitlement exists in the default config
	contactsEnt := macgo.Entitlement(entitlements.EntAddressBook)

	// The entitlement should be present and set to true
	if val, exists := macgo.DefaultConfig.Entitlements[contactsEnt]; !exists {
		t.Error("Contacts entitlement should be registered after package import")
	} else if !val {
		t.Error("Contacts entitlement should be set to true after package import")
	}
}

func TestMultipleImports(t *testing.T) {
	// Create a backup of the original config
	originalConfig := macgo.DefaultConfig
	defer func() {
		macgo.DefaultConfig = originalConfig
	}()

	// Reset config
	macgo.DefaultConfig = macgo.NewConfig()

	// Simulate multiple registrations (like multiple imports)
	macgo.RequestEntitlement(entitlements.EntAddressBook)
	macgo.RequestEntitlement(entitlements.EntAddressBook)
	macgo.RequestEntitlement(entitlements.EntAddressBook)

	// Should still work correctly
	contactsEnt := macgo.Entitlement(entitlements.EntAddressBook)
	if val, exists := macgo.DefaultConfig.Entitlements[contactsEnt]; !exists || !val {
		t.Error("Contacts entitlement should remain registered and true after multiple registrations")
	}
}

func TestDocumentationExample(t *testing.T) {
	// The documentation shows: import _ "github.com/tmc/misc/macgo/entitlements/contacts"
	// This should enable contacts access by registering the entitlement during init()

	originalConfig := macgo.DefaultConfig
	defer func() {
		macgo.DefaultConfig = originalConfig
	}()

	// Reset config to simulate a fresh import
	macgo.DefaultConfig = macgo.NewConfig()

	// Simulate what init() does
	macgo.RequestEntitlement(entitlements.EntAddressBook)

	// Verify the entitlement is registered
	contactsEnt := macgo.Entitlement(entitlements.EntAddressBook)
	if val, exists := macgo.DefaultConfig.Entitlements[contactsEnt]; !exists || !val {
		t.Error("Contacts entitlement should be registered and enabled after init simulation")
	}
}
